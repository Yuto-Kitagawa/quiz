...
<body>
    ...
    <main class="d-flex">
        ...
        <section id="mainSection" class="col-12 col-sm-12 col-md-12 col-lg-9 main-section p-2">
            <div class="main-container bg-white mt-3 p-1">
                ...
                <div class="mt-5 lead display-6 text-center">
                    問題編集
                </div>
                <div class="lead text-center text-success px-2" id="success" hidden>更新が完了しました。</div>
                <div class="lead text-center text-danger px-2" id="false" hidden>更新に失敗しました。ページを更新してやり直してください。</div>
                <div class="" id="edit">
                    <div class="col-10 m-auto mt-5">
                        <textarea name="quiz" id="quiz" class="form-control"></textarea>
                    </div>
                    ...
                    <div class="mt-4 col-10 col-sm-10 col-md-8 col-lg-6 m-auto">
                        <button class="btn btn-outline-dark col-12" id="refreshBtn">更新</button>
                    </div>
                </div>

            </div>
        </section>
        <div class="load" id="loadTmp">
    ...
    <script>
        function getQuiz() {
            ...
            const qid = "<?= $quiz_id ?>"
            fd.append('qid', qid);
            xhr.open('POST', "./action/get-quiz-one-all.php");
            xhr.send(fd);
            xhr.onload = (e) => {
                let json = e.target.response;
                let res = JSON.parse(json);
                document.getElementById('quiz').value = res[0];
                document.getElementById('answer').value = res[1];
        ...
        document.getElementById('refreshBtn').addEventListener('click', () => {
            ...
            fd2.append('quiz', quiz);
            fd2.append('answer', answer);
            fd2.append('qid', "<?= $quiz_id ?>");
            xhr2.open('POST', "./action/refresh-quiz.php");
            xhr2.send(fd2);
            xhr2.onload = (e) => {
                let json = e.target.response;
                let res = JSON.parse(json);
