    ...
    <body>
        ...
        <main class="d-flex">
            ...
            <section id="mainSection" class="col-12 col-sm-12 col-md-12 col-lg-9 main-section p-2">
                <div class="title main-container bg-white mt-3 p-4">
                    <div class="lead text-center">問題集の名前を入力してください</div>
                    <div class="text-center lead fs-6 mt-2">※ほかのタイトルとかぶらないようにしてください。</div>
                    <div class="text-center lead fs-6 ">※わかりやすいタイトルをおすすめします。</div>
                    <div class="text-center">
                        <small class="text-danger" id="warning" hidden>違うタイトルにしてください。</small>
                    </div>
                    <div class="py-3">
                        <div class="col-10 m-auto">
                            <input type="text" name="t" id="title" class="form-control" required autofocus>
                            <small class="small text-danger" id="err" hidden>入力してください。</small>
                        </div>

                        <div class="d-flex justify-content-around text-center p-3">
                            <a href="../index.php" class="btn btn-outline-danger col-3">戻る</a>
                            <button type="submit" id="submit" class="btn btn-outline-primary col-3">次へ</button>
                        </div>
                    </div>
                </div>
            </section>
        ...
        <script>
            document.getElementById('submit').addEventListener('click', () => {
                ...
                if (title != "") {
                    ...
                    xhr.open('post', "./../action/check-title.php");
                    xhr.send(fd);
                    xhr.onload = (e) => {
                        const res = e.target.response;
                        console.log(res);
                        if (JSON.parse(res) == "True") {
                            xhr.open('post', "./../action/settitle.php");
                            xhr.send(fd);
                            xhr.onload = (e) => {
                                window.location.href = "./question-and-answer.php";
                            }
                        } else {
